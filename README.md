...
<body>
  ...
  <div id="abaCronograma" class="aba">
    <div class="card">
      <h2>Cronograma</h2>
      <div class="subtitulo">Mistura um pouco de cada matéria por dia.</div>
...
<script>
  ...
  function montarBlocoMisto(indiceBase, titulo) {
    let bloco = document.createElement("div");
    bloco.className = "artigo";

    if (artigos.length === 0) {
      bloco.innerHTML = "<b>" + titulo + "</b><br>Nenhum artigo cadastrado.";
      return bloco;
    }
    ...
    artigos.forEach(function(a) {
      if (!porMateria[a.materia]) {
        porMateria[a.materia] = [];
      }
      porMateria[a.materia].push(a);
    });

    let materias = Object.keys(porMateria);
    ...
    let porMateriaNoDia = Math.max(1, Math.floor(metaDia / qtdMaterias));
    let resto = metaDia % qtdMaterias;

    let html = "<b>" + titulo + "</b><br><br>";
    let totalNoBloco = 0;

    materias.forEach(function(materia, idx) {
      let cota = porMateriaNoDia + (idx < resto ? 1 : 0);
      let inicioMateria = indiceBase * cota;
      let selecionados = porMateria[materia].slice(inicioMateria, inicioMateria + cota);

      if (selecionados.length > 0) {
        totalNoBloco += selecionados.length;
        html += "<b>" + materia + "</b><br>";
        selecionados.forEach(function(a) {
          html += "• " + a.artigo.substring(0, 120) + "...<br>";
        });
        html += "<br>";
      }
    });
  ...
  function mostrar() {
    ...
    let indiceHoje = progresso;

    if (dataInicial) {
      let diasPassados = diferencaDias(dataInicial, hoje());
      if (diasPassados < 0) diasPassados = 0;
      indiceHoje = diasPassados;
    }

    hojeDiv.innerHTML = "<p><b>Data de hoje:</b> " + hoje() + "</p>";
    hojeDiv.appendChild(montarBlocoMisto(indiceHoje, "Hoje"));

    proximosDiasDiv.appendChild(montarBlocoMisto(indiceHoje + 1, "Amanhã"));
    proximosDiasDiv.appendChild(montarBlocoMisto(indiceHoje + 2, "Depois de amanhã"));
    proximosDiasDiv.appendChild(montarBlocoMisto(indiceHoje + 3, "Próximo bloco"));

    let revisoesHoje = artigos.filter(function(a) {
      return a.dataRevisao <= hoje();
    });

    if (revisoesHoje.length === 0) {
      revisaoDiv.innerHTML = "<p>Nada para revisar hoje.</p>";
    } else {
      revisoesHoje.forEach(function(a) {
